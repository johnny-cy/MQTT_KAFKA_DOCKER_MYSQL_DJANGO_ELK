...
class CircleScorer(object):
    ...
    def calculate_score(self, df, column="pm2_5", threshold=54):
        ...
        self.score = score
        self._data_instance.score = self.score
        self._dirty = True
    ...
    def load(self):
        ...
        if self._data_instance is None:
            ...
            self.score = self._data_instance.score
...
class CircleController(object):
    ...
    def _append_circle_with_time(self, time):
        ...
        df["time"] = time
        df["circle"] = None
        self.df_circles = self.df_circles.append(
            df, sort=False)[["time", "circle_idx", "lat", "lon", "circle"]]
    ...
    def evaluate_batch(self, df_rawdata, threshold=54):
        try:
            ...
            return
        logger.info(f"evaluate batch: {self.area.name}, data length: {len(df_sub_data)}, circles: {len(df_circles)}")
        for circle in df_circles["circle"]:
    ...
    def cleanup(self):
        _2_hours_ago = datetime.datetime.now() - datetime.timedelta(hours=2)
        df = self.df_circles
...
def batch():
    ...
    while t > first_day:
        ...
        logger.info(f"{start_time} - {end_time}: length={len(df)}")

        if df.empty:
            continue

        controllers = [
            CircleController(area=Taichung, item="pm2_5"),
            CircleController(area=NewTaipei, item="pm2_5"),
            CircleController(area=Yilan, item="pm2_5"),
            CircleController(area=Taoyuan, item="pm2_5"),
            CircleController(area=Hsinchu, item="pm2_5"),
            CircleController(area=Miaoli, item="pm2_5"),
            CircleController(area=Yunlin, item="pm2_5"),
            CircleController(area=Chiayi, item="pm2_5"),
            CircleController(area=Tainan, item="pm2_5"),
            CircleController(area=Kaohsiung, item="pm2_5"),
            CircleController(area=Pingtung, item="pm2_5"),
        ]
        for controller in controllers:
