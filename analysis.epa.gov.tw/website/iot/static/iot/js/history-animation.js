...
passParams.end = maxDate.yymmddHHmm();
...
dataFetchPromises.push(
	Promise.resolve($.getJSON(API_URLS.iot_rawdata + '?' + encodeParamPairs(requestRawdataParam)))
		.then( function(res){
			// pre-formatting
			res.data && res.data.forEach(function(item){
				var parsedScore = parseFloat(item["pm2_5"]);
				item.score = isNaN(parsedScore) ? null : fixedFloat(parsedScore);
				item.datetime = item.time.replace('T', ' ').replace('Z', '');
			});
